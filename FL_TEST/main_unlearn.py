...
parser.add_argument('--mode',type =str, default="xx")
parser.add_argument('--alpha_FL_dis',type =float, default=1.0)
parser.add_argument('--save_model_file',type =str, default='./FL_TEST/FL_un_model_file/')
parser.add_argument('--save_data_file',type =str, default='./FL_TEST/acc_un/')
...
Un_test_dataloder = DataLoader(dataset=Subset(test_dataset, [i for i, (_, sample_label) in enumerate(test_dataset) if sample_label in params.class_unlearned])
, batch_size=128, shuffle=False, num_workers=0, drop_last=False)
Pre_test_dataloder = DataLoader(dataset=Subset(test_dataset, [i for i, (_, sample_label) in enumerate(test_dataset) if sample_label not in params.class_unlearned])
, batch_size=128, shuffle=False, num_workers=0, drop_last=False)
...
if params.client_number == 10 and params.alpha_FL_dis == 1.0:
    if params.dataset_name == "FMNIST":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/FMNIST_Lenet5_50.pth')
    if params.dataset_name == "PLANT":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/PLANT_MobileNetV2_50.pth')
    if params.dataset_name == "CIFAR":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/CIFAR_ResNet_100.pth')
    if params.dataset_name == "VGGFACE":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/VGGFACE_DenseNet_50.pth')
    if params.dataset_name == "SVHN":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/SVHN_VGG11_50.pth')

if params.client_number == 20 and params.alpha_FL_dis == 1.0:
    if params.dataset_name == "FMNIST":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/FMNIST_Lenet5_50_clientnum2050_FLalpha1.pth')
    if params.dataset_name == "CIFAR":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/CIFAR_ResNet_100_clientnum20100_FLalpha1.pth')
    if params.dataset_name == "VGGFACE":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/VGGFACE_DenseNet_50_clientnum2050_FLalpha1.pth')
    if params.dataset_name == "SVHN":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/SVHN_VGG11_50_clientnum2050_FLalpha1.pth')

if params.client_number == 10 and params.alpha_FL_dis == 2.0:
    if params.dataset_name == "FMNIST":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/FMNIST_Lenet5_50_clientnum1050_FLalpha2.0.pth')
    if params.dataset_name == "CIFAR":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/CIFAR_ResNet_100_clientnum10100_FLalpha2.0.pth')
    if params.dataset_name == "VGGFACE":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/VGGFACE_DenseNet_50_clientnum1050_FLalpha2.0.pth')
    if params.dataset_name == "SVHN":
        unlearn_model_dict = torch.load('./FL_TEST/FL_model_file/SVHN_VGG11_50_clientnum1050_FLalpha2.0.pth')
...
def FL_local_train():
    ...
    for i in choose_client:
        client_update_model_parameter,data_len = i.train_B()
...
nick_name = params.dataset_name + '_' + params.mode + "_epoch"+ str(params.global_epochs) + '_clientnum'+str(params.client_number)+str(params.global_epochs)+'_FLalpha'+str(params.alpha_FL_dis)+ ".pth"
torch.save(last_acc,params.save_data_file + nick_name)
if params.mode == "nearst_label_reweight":
