...
class SVHN_traindata(Dataset):
    def __init__(self):
        ...
        self.data = np.stack([self.trainset[i][0] for i in range(len(self.trainset))])
        self.label = np.array([self.trainset[i][1] for i in range(len(self.trainset))])
        # print(self.label)
        # print(self.label)
...
def return_data(params):
    ...
    if dataset == "CIFAR100":
        train_all_transforms = transforms.Compose([transforms.RandomCrop(32, padding=4),transforms.RandomHorizontalFlip(),transforms.ToTensor(),transforms.Normalize((0.4914, 0.4822, 0.4465), (0.247, 0.2434, 0.2610))])
        test_all_transforms = transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.4914, 0.4822, 0.4465), (0.247, 0.2434, 0.2610))])
        trainset = datasets.CIFAR100(root='/hy-tmp/', train=True, download=True, transform=train_all_transforms)
        testset = datasets.CIFAR100(root='/hy-tmp/', train=False, download=True, transform=test_all_transforms)
    if dataset == "SVHN":
